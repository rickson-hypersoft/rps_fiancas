...
@section('content')
    <div class="col-md-12">
        <form action="{{ $action }}" method="POST" class="fv-plugins-bootstrap5 fv-plugins-framework mb-0"
                        ...
                        @if ($method === 'PUT')
                            <div class="form-check form-switch mb-0">
                                <input class="form-check-input" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                    ...
                                    name="ativo" {{ $financialAccount['ativo'] ? 'checked' : '' }}>
                            </div>
                        @endif
                    </div>
                    <hr class="mt-0 pt-0">
                ...
                <div class="card-body">
                    ...
                    <div class="row gy-4 gx-6">
                        <div class="col-md-4 form-control-validation fv-plugins-icon-container">
                            ...
                            <select class="form-select form-select-lg" name="tipo_conta" id="tipo_conta"
                                aria-label="Default select example">
                                <option value="Conta Caixa"
                                    {{ old('tipo_conta', $financialAccount['tipo_conta'] ?? '') == 'Conta Caixa' ? 'selected' : '' }}>
                                    Conta Caixa
                                </option>
                                <option value="Conta Bancária"
                                    {{ old('tipo_conta', $financialAccount['tipo_conta'] ?? '') == 'Conta Bancária' ? 'selected' : '' }}>
                                    Conta Bancária
                                </option>
                                <option value="Outros"
                                    {{ old('tipo_conta', $financialAccount['tipo_conta'] ?? '') == 'Outros' ? 'selected' : '' }}>
                                    Outros
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input value="{{ old('descricao', $financialAccount['descricao'] ?? '') }}" type="text"
                                class="form-control form-control-lg" id="descricao" name="descricao" maxlength="100">
                        </div>

                        <div class="col-md-4" style="{{ $mostrarBanco ? '' : 'display: none;' }}">
                            <label for="banco_titular" class="form-label">Titular</label>
                            <input value="{{ old('banco_titular', $financialAccount['banco_titular'] ?? '') }}"
                                type="text" class="form-control form-control-lg" id="banco_titular" name="banco_titular"
                                maxlength="100">
                        </div>

                        <div class="col-md-4" style="{{ $mostrarBanco ? '' : 'display: none;' }}">
                            <label for="banco_cnpj" class="form-label">CNPJ</label>
                            <input value="{{ old('banco_cnpj', $financialAccount['banco_cnpj'] ?? '') }}" type="text"
                                class="form-control form-control-lg" id="banco_cnpj" name="banco_cnpj" maxlength="100">
                        </div>

                        <div class="col-md-4" style="{{ $mostrarBanco ? '' : 'display: none;' }}">
                            <label for="banco" class="form-label">Código do Banco</label>
                            <input value="{{ old('banco', $financialAccount['banco'] ?? '') }}" type="text"
                                class="form-control form-control-lg" id="banco" name="banco" maxlength="3">
                        </div>
                        ...
                        <div class="col-md-4" style="{{ $mostrarBanco ? '' : 'display: none;' }}">
                            ...
                            <select class="form-select form-select-lg" name="banco_finalidade" id="banco_finalidade"
                                aria-label="Default select example">
                                <option value="">Selecionar finalidade</option>
                                <option value="Inadimplência e Comissão"
                                    {{ old('banco_finalidade', $financialAccount['banco_finalidade'] ?? '') == 'Inadimplência e Comissão' ? 'selected' : '' }}>
                                    Inadimplência e Comissão
                                </option>
                                <option value="Inadimplência"
                                    {{ old('banco_finalidade', $financialAccount['banco_finalidade'] ?? '') == 'Inadimplência' ? 'selected' : '' }}>
                                    Inadimplência
                                </option>
                                <option value="Comissão"
                                    {{ old('banco_finalidade', $financialAccount['banco_finalidade'] ?? '') == 'Comissão' ? 'selected' : '' }}>
                                    Comissão
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4" style="{{ $mostrarBanco ? '' : 'display: none;' }}">
                            ...
                            <input value="{{ old('banco_pix', $financialAccount['banco_pix'] ?? '') }}" type="text"
                                class="form-control form-control-lg" id="banco_pix" name="banco_pix" maxlength="100">
                        </div>
                        ...
                        <div class="mt-4">
                            <button type="submit"
                                class="btn btn-primary waves-effect waves-light me-3">{{ $method == 'PUT' ? 'Salvar alterações' : 'Gravar' }}</button>
